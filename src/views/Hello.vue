<template>
  <div class="helloFirst">
    <div class="margin flex seachI">
      ...
      <el-button type="primary" round icon="el-icon-upload2" style="width:160px;" @click="upLoadimg()">上传</el-button><span>或</span>
...
<!--    <img :src="test" alt="">-->
    <div class="margin">
      ...
      <div class=" flex j-b content" v-show="canvasShow">
        <i style="position: absolute;right: 10px;top: 10px;font-size: 24px;color: #999;" class="el-icon-close cu" @click="clearFile()"></i>
          <div class="left">
            <div class="img">
<!--              <img :src="other" alt="" v-if="!backimgShow">-->
              ...
              <div v-show="backimgShow" class="imgList">
                ...
                <div class="titleList">
                  <span v-for="(val,index) in titleList" :key="index" :class="{'active': index==titleIndex}" class="cu" @click="changeimgList(index)">{{val}}</span>
                </div>
                <div class="imgAll flex">
                  <img :src="item" alt="" v-for="(item,index) in imgAllList[titleIndex]" :key="index" @click="addBackImg(item)" class="cu">
                </div>
                <el-button type="primary" round style="display:block;width:100px; margin: 50px auto;" size="small" @click="backimgShow=false">确定</el-button>
              </div>
            </div>
          </div>
          <div class="right">
            <div class="canvas" @mousedown="mouseDown($event)" @mousemove.prevent="moeseMove($event)" @mouseup="mouseUp($event)">
              ...
              <div class="border" v-show="borderShow" :style="{'top':borderLocal.y,'left':borderLocal.x,'width':borderLocal.width,'height': borderLocal.height}"></div>
            </div>
            <div class="btns flex">
<!--              <el-button round>圆角按钮</el-button><el-button round icon="el-icon-edit">圆角按钮</el-button>-->
              <span>前景</span>
              <div class="cu btn" :class="{'active':clearOrdraw}"  @click="clearOrdraw=true"  slot="reference"><i class="icon iconfont icon-xiangpi"></i>擦除</div>
              <div class="cu btn" :class="{'active':!clearOrdraw}" @click="clearOrdraw=false"><i class="icon iconfont icon-qianbi"></i>还原</div>
              <div class="cu re" @click="backReset()"><i class="icon iconfont icon-chexiaofanhuichehuishangyibu"></i>撤回</div>
            ...
            <div style="padding-left: 20px;line-height: 38px;" class="flex">
              <div style="width:200px;margin-right: 10px;"><el-slider v-model="eraserWidthSelect" tooltip-effect="light" :min="3" :max="25" :show-tooltip="showThost"></el-slider></div><span>{{eraserWidthSelect}}px</span>
            </div>
            <div class="otherBtn flex a-i">
              <span>背景</span>
              <div v-for="(item,index) in color" :key="index" :style="backg" @click="choseBackColor(item,index)" class="cu" :class="{'bordershow' : index==choseBack}" v-if="index===0"></div>
              <div v-for="(item,index) in color" :key="index" :style="{'background':item }" @click="choseBackColor(item,index)" class="cu" :class="{'bordershow' : index==choseBack}" v-if="index!==0"></div>
<!--              <el-button round icon="el-icon-more" size="mini" @click="choseColor()"></el-button>-->
                <span style="height: 28px;"><el-color-picker v-model="colorValue" size="mini" ref="colorS" @change="choseColor()" :class="{'bordershow': choseBack=='span'}"></el-color-picker></span>
              <p :style="back" class="cu" @click="backimgShow=!backimgShow"><i class="icon iconfont icon-zhaoxiangji"></i>背景图片</p>
            </div>
            <el-button type="primary" round icon="el-icon-download" style="width:160px;margin: 50px 0 0 150px;" @click="save()">下载</el-button>
          </div>
      </div>
    </div>
    <div class="section">
...
<!--          <input type="file" style="display: none" ref="upImg" @change="changeImg($event)">-->
          <el-button type="primary" round icon="el-icon-upload2" style="width:160px;" @click="upLoadimgDwon()">上传</el-button><span>或</span>
          <div class="flex input">
            <input placeholder="粘贴URL地址" v-model="imgUrl"/>
            <div class="cu" @click="copyImgUrl()">确定</div>
          </div>
...
export default {
  ...
  methods:{
    ...
    choseColor(){//选择颜色背景
      ...
      this.cDownTxt.fillRect(0,0,this.cDown.width,this.cDown.height)
    },
    ...
    getImgData(url,callback){//解决跨域问题
     ...
     save(){//保存下载
       ...
       if(this.choseBack!==0){
         let dataURL1 = this.cDown.toDataURL("image/png");
         let background=new Image()
         background.src=dataURL1
    ...
    saveTwo(can,ctx){//保存方法提取
      let dataURL =this.cUp.toDataURL("image/png");
      let imgA=new Image()
      imgA.src=dataURL
      imgA.crossOrigin = ""
      imgA.onload=(e)=>{
    ...
    copyImgUrl(){//粘贴图片链接确定
      ...
      axios({
        ...
        if(res.data.code==0){
          ...
          this.drawImg(res.data.data.original,[this.hiddenCanvas])
          this.drawImg(this.opacity,[this.cDown])
          // this.upImgUrl=res.data.data.bgRemoved
          this.drawImg(res.data.data.bgRemoved,[this.cUp],'closeLoading')
    ...
    drawImg(url,arr,close,clear){//画图封装
      ...
      image.onload=function(ev) {
        ...
        console.log(250-image.width/2, 250-image.height/2,image.width,image.height)
        arr.map((obj, index) => {
          obj.getContext('2d').drawImage(image, 250-image.width/2, 250-image.height/2,image.width,image.height);
        })
...
//	     		alert("文件大小:"+(file.size / 1024).toFixed(1)+"kB")
    ...
    mouseDown(e){
      // if(this.cUp.toDataURL()!= this.historySet[this.historySet.length]){
        this.historySet.push(this.cUp.toDataURL())
      // }
    ...
    addBackImg(item){//添加背景图片
      ...
      image.crossOrigin = ""
      image.onload=function(ev) {
          if(image.height>500) y=(image.height-500)/2
          else y=0
        if(image.width>500) x=(image.width-500)/2
          else x=0
        console.log(x,y,500,500,0,0,500,500,'-----',image.width,image.height)
        _self.cDownTxt.drawImage(image,x,y,500,500,0,0,500,500);
      }
      image.src=item
    },
  }
